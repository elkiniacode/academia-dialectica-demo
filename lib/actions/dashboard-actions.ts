...
const _getDashboardKPIs = unstable_cache(
  async (year: number): Promise<MonthlyKPI[]> => {
...
export async function getDashboardKPIs(year: number): Promise<MonthlyKPI[]> {
  const session = await auth();
  if (!session || session.role !== "ADMIN") return [];
  return _getDashboardKPIs(year);
}

const _getClientRevenueMatrix = unstable_cache(
  async (year: number): Promise<ClientRevenueMatrix> => {
    const balances = await prisma.monthlyBalance.findMany({
      where: { year },
      include: { entries: true },
    });
  ...
  ["dashboard-revenue-matrix"],
  { revalidate: 3600, tags: ["dashboard"] }
);

export async function getClientRevenueMatrix(year: number): Promise<ClientRevenueMatrix> {
  const session = await auth();
  if (!session || session.role !== "ADMIN") {
    return { clients: [], data: {}, monthTotals: Array(12).fill(0), clientTotals: {}, grandTotal: 0 };
  }
  return _getClientRevenueMatrix(year);
}

const _getClientGrowthData = unstable_cache(
  async (year: number): Promise<ClientGrowthMonth[]> => {
...
export async function getClientGrowthData(year: number): Promise<ClientGrowthMonth[]> {
  const session = await auth();
  if (!session || session.role !== "ADMIN") return [];
  return _getClientGrowthData(year);
}

const _getYearComparisonData = unstable_cache(
  async (): Promise<YearComparisonData> => {
