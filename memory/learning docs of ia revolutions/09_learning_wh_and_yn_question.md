...
## 2. Never Use `.optional()` with OpenAI Structured Output

OpenAI's structured output API requires every property in `properties` to also be in the `required` array. Zod's `.optional()` removes the field from `required`, causing a runtime 400 error that the TypeScript compiler cannot catch.

**Rule:** Never use `.optional()` in Zod schemas passed to `streamObject` with OpenAI. Use `.nullable()` when a field may not have a value. Always set nullable fields explicitly (`null`, not omitted). Design schemas for the AI consumer (OpenAI), not for TypeScript idioms.

```typescript
// Wrong: excluded from "required", OpenAI rejects
questionWord: z.string().optional()

// Correct: stays in "required", accepts null
questionWord: z.string().nullable()
```

## 3. Lock Down AI-Generated Discrete Values
...
## 4. Evidence-First Schema Ordering
...
## 5. Use `keyof` to Close Data-to-UI Mapping
...
## 6. Separate Setup Data from Response Data
...
## 7. Ref Guard Before Side Effect
...
## 8. Multi-Round Ref Locks Need Reset
...
## 9. Don't Break Out of the Store Action Pattern
...
## 10. Derive Terminal Conditions from Structure
...
## 11. Ban Impossible Structures in the Prompt
...
## 12. Instruction Values Must Match Schema Literals
...
## 13. Restrict Options to Structurally Compatible Values
...
## 14. Disable Browser Assistance on Learning Inputs
...
## 15. Auto-Focus Frustration Paths
...
## 16. Keyboard Shortcuts for Multi-Line Inputs
...
## 17. Reserve Space for Indicators — Toggle Opacity, Not Existence
...
## 18. Per-Element Opacity for Disabled States
...
## 19. Visual Feedback During Delays is Mandatory
...
## 20. Stagger Reveals for Dramatic Moments
...
## 21. Cross-Reference Linking Between Lists
...
## 22. No Dynamic Tailwind Class Construction
...
## 24. Feature Gates on Business Rules, Not Phase Names
...
## 25. Filter System Messages Before API Calls
...
## 26. Fire-and-Forget Still Needs Catch
...
## 27. HP-Zero Must Trigger Gameover Everywhere
